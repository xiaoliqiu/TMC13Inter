...
attr="predlift"
src_cfg_dir=""
ra_cfg="cfg-inter-ra=8f.yaml"

while (( $# )); do
	case $1 in
	--intra) ra_cfg="cfg-intraseq.yaml" ;;
	--inter) ra_cfg="cfg-inter-ra=8f.yaml" ;;
	--octree) geom="octree" ;;
	...
	--help|*)
		echo -e "usage:\n $0\n" \
			"    [[--intra|--inter] [--octree|--trisoup] [--raht|--pred-lift] | --all]\n" \
			"    [--cfgdir=<dir>]"
		exit 1
	esac
	shift;
...
do_one_cfgset() {
	...
	for f in ${!cfgset}
	do
		echo "${src_cfg_dir}$f ${src_cfg_dir}${ra_cfg} -> $outdir" ...
	done
